...
namespace ProjectTime.HexGrid
{
    ...
    public class SnapToHexGrid : MonoBehaviour
    {
        ...
        {
            var xInt = Mathf.RoundToInt(((position.x + 1f) - (position.z * .5f) + (position.z / 2)) / (Hex.innerRadius * 2f));
            if (position.z > 0)
                xInt = Mathf.RoundToInt(((position.x - 1f) - (position.z * .5f) + (position.z / 2)) / (Hex.innerRadius * 2f));
